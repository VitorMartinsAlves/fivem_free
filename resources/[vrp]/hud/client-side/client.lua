...
function updateDisplayHud()
	...
	if IsPedInAnyVehicle(ped) then
		...
		SendNUIMessage({ vehicle = false, talking = talking, health = health, armour = armour, thirst = thirst, hunger = hunger, stress = stress, stamina = stamina, oxigen = clientOxigen, suit = divingMask, street = streetName, direction = flexDirection, radio = radioDisplay, voice = voice, time = horario })
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- HUD
	...
	while true do
		...
		if GetEntityHealth(ped) > 101 then
			if hunger >= 6 and hunger <= 15 then
				...
				TriggerEvent("Notify","aviso","Sofrendo com a fome.",5000)
			...
			if thirst >= 6 and thirst <= 15 then
				ApplyDamageToPed(ped,1,false)
				TriggerEvent("Notify","aviso","Sofrendo com a sede.",5000)
